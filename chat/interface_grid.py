    ...
    def run(self):
        while 1:
            ...
            if message_recu == '' or message_recu.upper() == 'FIN':
                break
            self.liste.insert(END, nom + message_recu) # Affichage du message
...
class Chat(object):
    ...
    def envoi(self):
        ...
        self.liste.insert(END, message)

    def main(self):
        self.liste = Listbox(self.fenetre)
        self.liste.grid(row=0,column=0, columnspan=4, sticky=W+E+N+S)
        ...
        self.bouton_fermer=Button(self.fenetre, text="Fermer", command=self.fenetre.quit)
        self.bouton_fermer.grid(row=1,column=3)
...
if __name__ == '__main__':
    test = Accueil()
#fuuuuuusion !    columnspan=2, rowspan=2